...
const manoObraService = {
  /**
   * Obtener lista de mano de obra con filtros y paginación
   */
  async getAll(params?: GetAllParams): Promise<ApiResponse<ManoObra[]>> {
    ...
    const response = await client.get<ApiResponse<ManoObra[]>>('/construccion/mano-obra', {
      params: queryParams,
    });
   ...
   */
  async getById(id: number): Promise<ApiResponse<ManoObra>> {
    const response = await client.get<ApiResponse<ManoObra>>(`/construccion/mano-obra/${id}`);
    return response.data;
  },
   ...
   */
  async create(data: ManoObraFormData): Promise<ApiResponse<ManoObra>> {
    const payload = {
    ...
    const response = await client.post<ApiResponse<ManoObra>>('/construccion/mano-obra', payload);
    return response.data;
  },

  /**
   * Actualizar mano de obra existente
   */
  async update(id: number, data: ManoObraFormData): Promise<ApiResponse<ManoObra>> {
    const payload = {
      descripcion: data.descripcion,
      unidad_id: data.unidad_id,
      apu_item_id: data.apu_item_id,
    ...
    const response = await client.put<ApiResponse<ManoObra>>(`/construccion/mano-obra/${id}`, payload);
    return response.data;
  },
  ...
  async toggleEstado(id: number): Promise<ApiResponse<{ id: number; descripcion: string; activo: boolean }>> {
    const response = await client.patch<ApiResponse<{ id: number; descripcion: string; activo: boolean }>>(
      `/construccion/mano-obra/${id}/toggle-estado`
    );
    return response.data;
  },
...
export default manoObraService;