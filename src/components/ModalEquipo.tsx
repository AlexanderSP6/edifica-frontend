...
import { equipoValidationSchema, getInitialValues } from '../schemas/equipoValidationSchema';
...
const ModalEquipo: React.FC<ModalEquipoProps> = ({
  open,
  onClose,
  onSave,
  itemEdit,
  ...
  const formik = useFormik({
    ...
    validationSchema: equipoValidationSchema,
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const errorMessage = err.response?.data?.message || 'Error al guardar el equipo';
      ...
      <DialogTitle sx={{ pb: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Typography variant="h6" component="span" sx={{ fontWeight: 600 }}>
            {itemEdit ? 'Editar Equipo' : 'Nuevo Equipo'}
          </Typography>
          <IconButton
            onClick={handleClose}
            disabled={loading}
          ...
          <Grid container spacing={3}>
            ...
            <Grid size={{ xs: 12 }}>
              <TextField
                fullWidth
                label="Descripción"
                placeholder="Ej: Excavadora, Compactadora, Herramientas menores..."
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField
                ...
                helperText={
                  getErrorText('rendimiento') || 'Hasta 4 decimales'
                }
                disabled={loading}
                inputProps={{ 
                  inputMode: 'decimal',
